...
class WPLA_Admin {
    ...
    public function wpla_save_settings(): void {
        ...
        wp_send_json_success( array(
            'message' => __( 'Configurações salvas com sucesso.', 'lc-crm' ),
            'api_key' => get_option( 'wpla_api_key' ),
        ) );
    }
    ...
    public function wpla_send_test_email(): void {
        ...
        $subject = isset( $_POST['subject'] ) ? sanitize_text_field( wp_unslash( $_POST['subject'] ) ) : 'Email de Teste do LC CRM';
    ...
    public function wpla_send_test_whatsapp(): void {
        ...
        $phone   = isset( $_POST['phone'] ) ? sanitize_text_field( wp_unslash( $_POST['phone'] ) ) : '';
        $message = isset( $_POST['message'] ) ? sanitize_text_field( wp_unslash( $_POST['message'] ) ) : 'Teste do LC CRM';
    ...
    public function wpla_get_webinar(): void {
        ...
        if ( ! $webinar ) {
            wp_send_json_error( array( 'message' => __( 'Webinar não encontrado.', 'lc-crm' ) ) );
            return;
        }
    ...
    public function wpla_save_webinar(): void {
        ...
        if ( $id ) {
            ...
            if ( ! $new_id ) {
                wp_send_json_error( array( 'message' => __( 'Erro ao criar webinar.', 'lc-crm' ) ) );
                return;
            }
    ...
    public function wpla_evolution_create_instance(): void {
        ...
        if ( empty( $api_url ) || empty( $api_key ) || empty( $instance ) ) {
            wp_send_json_error( array( 'message' => __( 'Preencha URL, chave API e nome da instância.', 'lc-crm' ) ) );
            return;
        }
        ...
        if ( $code >= 200 && $code < 300 || $code === 409 ) {
            ...
            $msg = isset( $body['message'] ) ? $body['message'] : __( 'Erro ao criar instância Evolution.', 'lc-crm' );
    ...
    public function wpla_evolution_get_qr(): void {
        ...
        if ( empty( $api_url ) || empty( $api_key ) || empty( $instance ) ) {
            wp_send_json_error( array( 'message' => __( 'Configurações da Evolution API incompletas.', 'lc-crm' ) ) );
            return;
        }
