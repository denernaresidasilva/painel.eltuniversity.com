...
class PLS_Rest_Api {
    ...
    public static function register_routes(): void {
        ...
        register_rest_route( 'pls/v1', '/webhook/inbound', array(
        ...
        register_rest_route( 'pls/v1', '/webhook', array(
            'methods'             => 'GET,HEAD,POST',
            'callback'            => array( __CLASS__, 'handle_webhook' ),
            'permission_callback' => array( __CLASS__, 'validate_api_key' ),
        ) );
        ...
        register_rest_route( 'pls/v1', '/webinar/event', array(
            'methods'             => 'POST',
        ...
        register_rest_route( 'pls/v1', '/contacts', array(
            array(
                'methods'             => 'GET',
        ...
        register_rest_route( 'pls/v1', '/contacts/(?P<id>\d+)', array(
            array(
                'methods'             => 'GET',
        ...
        register_rest_route( 'pls/v1', '/lists', array(
            'methods'             => 'GET',
            'callback'            => array( __CLASS__, 'get_lists' ),
            'permission_callback' => array( __CLASS__, 'admin_permission' ),
        ) );

        register_rest_route( 'pls/v1', '/tags', array(
            'methods'             => 'GET',
            'callback'            => array( __CLASS__, 'get_tags' ),
            'permission_callback' => array( __CLASS__, 'admin_permission' ),
        ) );

        register_rest_route( 'pls/v1', '/automations', array(
            'methods'             => 'GET',
            'callback'            => array( __CLASS__, 'get_automations' ),
            'permission_callback' => array( __CLASS__, 'admin_permission' ),
        ) );

        register_rest_route( 'pls/v1', '/automations/(?P<id>\d+)/steps', array(
            'methods'             => 'GET',
            'callback'            => array( __CLASS__, 'get_automation_steps' ),
            'permission_callback' => array( __CLASS__, 'admin_permission' ),
        ) );

        register_rest_route( 'pls/v1', '/stats', array(
            'methods'             => 'GET',
        ...
        register_rest_route( 'pls/v1', '/email-templates', array(
            array(
                'methods'             => 'GET',
        ...
        register_rest_route( 'pls/v1', '/email-templates/(?P<id>\d+)', array(
            array(
                'methods'             => 'GET',
        ...
        // Email statistics.
        register_rest_route( 'pls/v1', '/email-stats', array(
            'methods'             => 'GET',
    ...
    public static function handle_webhook( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( is_wp_error( $result ) && 'duplicate_email' === $result->get_error_code() ) {
            ...
            if ( $contact_id ) {
                PLS_Contact::update( (int) $contact_id, $data );
            }
        ...
        if ( ! empty( $params['tags'] ) ) {
            ...
            foreach ( $tags as $tag_name ) {
                ...
                $tag = PLS_Tag::get_by_name( $tag_name );
                if ( ! $tag ) {
                    $tag_id = PLS_Tag::create( array( 'name' => $tag_name ) );
                } else {
                    $tag_id = $tag->id;
                }
                PLS_Contact::add_tag( (int) $contact_id, (int) $tag_id );
            }
        }
        ...
        if ( $list_id_from_query > 0 ) {
            PLS_Contact::subscribe_list( (int) $contact_id, $list_id_from_query );
        }

        do_action( 'pls_event', 'webhook_received', (int) $contact_id, $params );
    ...
    public static function handle_webinar_event( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( ! $contact_id && is_email( $email ) ) {
            $contact = PLS_Contact::get_by_email( $email );
            if ( $contact ) {
        ...
        if ( $destination ) {
            PLS_Webinar::route_contact( $webinar_id, $contact_id, $destination );
        } else {
            // Unknown event — just fire action for custom handling.
            do_action( 'pls_event', "webinar_{$event}", $contact_id, array(
    ...
    public static function get_contacts( WP_REST_Request $request ): WP_REST_Response {
        $result = PLS_Contact::query( array(
    ...
    public static function get_contact( WP_REST_Request $request ): WP_REST_Response {
        $contact = PLS_Contact::get( (int) $request['id'] );
        if ( ! $contact ) {
            return new WP_REST_Response( array( 'error' => 'Not found' ), 404 );
        }
        $contact->tags  = PLS_Contact::get_tags( (int) $contact->id );
        $contact->lists = PLS_Contact::get_lists( (int) $contact->id );
        return new WP_REST_Response( $contact );
    }

    public static function create_contact( WP_REST_Request $request ): WP_REST_Response {
        $result = PLS_Contact::create( $request->get_json_params() );
    ...
    public static function update_contact( WP_REST_Request $request ): WP_REST_Response {
        $success = PLS_Contact::update( (int) $request['id'], $request->get_json_params() );
    ...
    public static function delete_contact( WP_REST_Request $request ): WP_REST_Response {
        $success = PLS_Contact::delete( (int) $request['id'] );
        return new WP_REST_Response( array( 'success' => $success ) );
    }

    public static function get_lists(): WP_REST_Response {
        return new WP_REST_Response( PLS_List_Model::all( '' ) );
    }

    public static function get_tags(): WP_REST_Response {
        return new WP_REST_Response( PLS_Tag::all() );
    }

    public static function get_automations(): WP_REST_Response {
        return new WP_REST_Response( PLS_Automation_Engine::all_automations() );
    }

    public static function get_automation_steps( WP_REST_Request $request ): WP_REST_Response {
        return new WP_REST_Response( PLS_Automation_Engine::get_steps( (int) $request['id'] ) );
    }
    ...
    public static function get_stats(): WP_REST_Response {
        ...
        return new WP_REST_Response( array(
            ...
            'email_stats'        => PLS_Email::get_stats(),
            'recent_contacts'    => PLS_Event_Manager::count_by_type( 'contact_created', $seven_days_ago ),
        ) );
    }
    ...
    public static function get_email_templates(): WP_REST_Response {
        return new WP_REST_Response( PLS_Email_Template::all() );
    }

    public static function get_email_template( WP_REST_Request $request ): WP_REST_Response {
        $tpl = PLS_Email_Template::get( (int) $request['id'] );
        if ( ! $tpl ) {
            return new WP_REST_Response( array( 'error' => 'Not found' ), 404 );
        }
    ...
    public static function create_email_template( WP_REST_Request $request ): WP_REST_Response {
        $params = $request->get_json_params();
        $id     = PLS_Email_Template::create( $params );
    ...
    public static function update_email_template( WP_REST_Request $request ): WP_REST_Response {
        $success = PLS_Email_Template::update( (int) $request['id'], $request->get_json_params() );
        return new WP_REST_Response( array( 'success' => $success ) );
    }

    public static function delete_email_template( WP_REST_Request $request ): WP_REST_Response {
        $success = PLS_Email_Template::delete( (int) $request['id'] );
        return new WP_REST_Response( array( 'success' => $success ) );
    }
    ...
    public static function get_email_stats( WP_REST_Request $request ): WP_REST_Response {
        ...
        return new WP_REST_Response( PLS_Email::get_stats( $since ) );
    }
}
