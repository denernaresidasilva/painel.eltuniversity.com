...
class PLS_Email {
    ...
    public static function send( object $message ): bool {
        ...
        $contact = PLS_Contact::get( (int) $message->contact_id );
        ...
        if ( (int) $message->template_id > 0 ) {
            $tpl = PLS_Email_Template::get( (int) $message->template_id );
            if ( $tpl ) {
                if ( empty( $subject ) ) {
                    $subject = $tpl->subject;
                }
        ...
        // Render merge fields.
        $body    = PLS_Email_Template::render( $body, $contact );
        $subject = $contact ? PLS_Email_Template::render( $subject, $contact ) : $subject;

        // Add unsubscribe link if not already present.
        if ( $contact && strpos( $body, 'pls_unsub' ) === false ) {
            $unsub_url  = add_query_arg( array(
                'pls_unsub' => '1',
                'email'     => rawurlencode( $contact->email ?? '' ),
                'token'     => PLS_Email_Template::unsubscribe_token( $contact->email ?? '' ),
            ), home_url( '/' ) );
            $body .= '<p style="font-size:11px;color:#9ca3af;text-align:center;margin-top:30px;">'
                   . '<a href="' . esc_url( $unsub_url ) . '" style="color:#9ca3af;">'
                   . esc_html__( 'Cancelar inscrição', 'plugin-leads-saas' )
                   . '</a></p>';
        }
        ...
        ob_start();
        $template_file = PLS_PLUGIN_DIR . 'templates/emails/default.php';
        ...
        $from_name  = get_option( 'pls_email_from_name', get_bloginfo( 'name' ) );
        $from_email = get_option( 'pls_email_from_address', get_bloginfo( 'admin_email' ) );
    ...
    private static function allow_send( int $contact_id ): bool {
        $max_per_hour = (int) get_option( 'pls_email_rate_limit', 3 );
        if ( $max_per_hour <= 0 ) {
            return true; // No limit configured.
        }
        $trans_key = 'pls_email_rate_' . $contact_id;
    ...
    public static function update_contact_email_meta( int $contact_id, string $event ): void {
        global $wpdb;
        $table = PLS_Database::table( 'contacts' );
     ...
     *
     * @deprecated Use PLS_Email_Template::render() instead.
     */
    public static function render_template( string $body, ?object $contact ): string {
        return PLS_Email_Template::render( $body, $contact );
    }
    ...
    private static function add_click_tracking( string $body, string $tracking_id ): string {
        return preg_replace_callback(
            ...
            function ( $matches ) use ( $tracking_id ) {
                ...
                if ( strpos( $url, 'pls_track' ) !== false || strpos( $url, 'pls_unsub' ) !== false ) {
                    return $matches[0];
                }
    ...
    public static function configure_smtp( $phpmailer ): void {
        $host = get_option( 'pls_smtp_host', '' );

        if ( empty( $host ) ) {
            return; // No SMTP configured — use WordPress default.
        }

        $port       = (int) get_option( 'pls_smtp_port', 587 );
        $encryption = get_option( 'pls_smtp_encryption', 'tls' );
        $auth       = (bool) get_option( 'pls_smtp_auth', '1' );
        $user       = get_option( 'pls_smtp_user', '' );
        $pass       = get_option( 'pls_smtp_pass', '' );
        $from_name  = get_option( 'pls_email_from_name', get_bloginfo( 'name' ) );
        $from_email = get_option( 'pls_email_from_address', get_bloginfo( 'admin_email' ) );
    ...
    public static function handle_tracking(): void {
        ...
        if ( isset( $_GET['pls_unsub'] ) && isset( $_GET['email'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
            ...
            if ( is_email( $email ) && PLS_Email_Template::verify_unsubscribe_token( $email, $token ) ) {
                // Rate limit unsubscribe processing: max 5 requests per IP per minute.
                $ip         = sanitize_text_field( $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0' );
                $rate_key   = 'pls_unsub_rate_' . md5( $ip );
                ...
                if ( $rate_count < 5 ) {
                    ...
                    if ( $contact ) {
                        ...
                        do_action( 'pls_event', 'email_unsubscribed', (int) $contact->id, array( 'email' => $email ) );
                    }
                }
            }
            ...
            wp_die(
                esc_html__( 'Você foi descadastrado com sucesso.', 'plugin-leads-saas' ),
                esc_html__( 'Descadastrado', 'plugin-leads-saas' ),
                array( 'response' => 200 )
            );
        }

        if ( ! isset( $_GET['pls_track'], $_GET['tid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
            return;
        }

        $type        = sanitize_text_field( wp_unslash( $_GET['pls_track'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
        $tracking_id = sanitize_text_field( wp_unslash( $_GET['tid'] ) ); // phpcs:ignore WordPress.Security.NonceVerification

        global $wpdb;
        $table = PLS_Database::table( 'message_queue' );
        ...
        if ( 'open' === $type ) {
            ...
            do_action( 'pls_event', 'email_opened', (int) $message->contact_id, array( 'tracking_id' => $tracking_id ) );
        ...
        if ( 'click' === $type && isset( $_GET['url'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
            ...
            do_action( 'pls_event', 'link_clicked', (int) $message->contact_id, array(
    ...
    public static function get_stats( string $since = '' ): array {
        ...
        $contacts_table        = PLS_Database::table( 'contacts' );
...
add_action( 'template_redirect', array( 'PLS_Email', 'handle_tracking' ), 1 );

// Configure SMTP via PHPMailer when settings are present.
add_action( 'phpmailer_init', array( 'PLS_Email', 'configure_smtp' ) );
