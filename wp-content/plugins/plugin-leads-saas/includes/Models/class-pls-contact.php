...
class PLS_Contact {
    ...
    public static function create( array $data ) {
        ...
        if ( empty( $email ) || ! is_email( $email ) ) {
            return new WP_Error( 'invalid_email', __( 'A valid email address is required.', 'plugin-leads-saas' ) );
        }
        ...
        if ( $existing ) {
            return new WP_Error( 'duplicate_email', __( 'A contact with this email already exists.', 'plugin-leads-saas' ), array( 'contact_id' => (int) $existing ) );
        }
        ...
        if ( false === $inserted ) {
            return new WP_Error( 'db_error', __( 'Could not create contact.', 'plugin-leads-saas' ) );
        }
         ...
         */
        do_action( 'pls_event', 'contact_created', $contact_id, $data );
    ...
    public static function update( int $id, array $data ) {
        ...
        if ( false !== $result ) {
            do_action( 'pls_event', 'contact_updated', $id, $data );
        }
    ...
    public static function delete( int $id ): bool {
        ...
        $wpdb->delete( PLS_Database::table( 'contact_tags' ), array( 'contact_id' => $id ), array( '%d' ) );
        $wpdb->delete( PLS_Database::table( 'contact_lists' ), array( 'contact_id' => $id ), array( '%d' ) );

        return (bool) $wpdb->delete( PLS_Database::table( 'contacts' ), array( 'id' => $id ), array( '%d' ) );
    }
    ...
    public static function query( array $args = array() ): array {
        ...
        if ( ! empty( $args['tag_id'] ) ) {
            $join    .= ' INNER JOIN ' . PLS_Database::table( 'contact_tags' ) . ' ct ON ct.contact_id = c.id';
            $where[]  = 'ct.tag_id = %d';
            $values[] = absint( $args['tag_id'] );
        }

        if ( ! empty( $args['list_id'] ) ) {
            $join    .= ' INNER JOIN ' . PLS_Database::table( 'contact_lists' ) . ' cl ON cl.contact_id = c.id';
            $where[]  = 'cl.list_id = %d';
    ...
    public static function add_tag( int $contact_id, int $tag_id ): bool {
        global $wpdb;
        $table = PLS_Database::table( 'contact_tags' );
        ...
        if ( $result ) {
            do_action( 'pls_event', 'tag_added', $contact_id, array( 'tag_id' => $tag_id ) );
        }
    ...
    public static function remove_tag( int $contact_id, int $tag_id ): bool {
        global $wpdb;
        $result = $wpdb->delete(
            PLS_Database::table( 'contact_tags' ),
            array( 'contact_id' => $contact_id, 'tag_id' => $tag_id ),
            array( '%d', '%d' )
        );

        if ( $result ) {
            do_action( 'pls_event', 'tag_removed', $contact_id, array( 'tag_id' => $tag_id ) );
        }
    ...
    public static function get_tags( int $contact_id ): array {
        global $wpdb;
        $ct = PLS_Database::table( 'contact_tags' );
        $t  = PLS_Database::table( 'tags' );
    ...
    public static function subscribe_list( int $contact_id, int $list_id ): bool {
        global $wpdb;
        $table = PLS_Database::table( 'contact_lists' );
        ...
        if ( $result ) {
            do_action( 'pls_event', 'list_subscribed', $contact_id, array( 'list_id' => $list_id ) );
        }
