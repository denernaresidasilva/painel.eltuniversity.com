...
class PLS_Webinar {
    ...
    public static function create( array $data ) {
        global $wpdb;
        $table = PLS_Database::table( 'webinars' );
        ...
        $automation_id = PLS_Automation_Engine::save_automation( array(
            'name'         => sprintf( __( 'Automação — %s', 'plugin-leads-saas' ), $row['name'] ),
            'description'  => sprintf( __( 'Automação dedicada do webinar "%s". Gerada automaticamente.', 'plugin-leads-saas' ), $row['name'] ),
        ...
        do_action( 'pls_webinar_created', $webinar_id );
    ...
    public static function get( int $id ): ?object {
        ...
        return $wpdb->get_row( $wpdb->prepare(
            'SELECT * FROM ' . PLS_Database::table( 'webinars' ) . ' WHERE id = %d',
            $id
        ) ) ?: null;
    }
    ...
    public static function delete( int $id ): bool {
        ...
        if ( $webinar->automation_id ) {
            PLS_Automation_Engine::delete_automation( (int) $webinar->automation_id );
        }

        // Remove webinar lists.
        $lists_table = PLS_Database::table( 'lists' );
        $lists       = $wpdb->get_results( $wpdb->prepare(
            "SELECT id FROM $lists_table WHERE webinar_id = %d",
            $id
        ) );
        foreach ( $lists as $list ) {
            PLS_List_Model::delete( (int) $list->id );
        }

        return (bool) $wpdb->delete( PLS_Database::table( 'webinars' ), array( 'id' => $id ), array( '%d' ) );
    }
    ...
    private static function create_lists( int $webinar_id, string $webinar_name ): void {
        foreach ( self::LIST_TYPES as $type => $label ) {
            PLS_List_Model::create( array(
                'name'       => "{$webinar_name} — {$label}",
                'description' => sprintf( __( 'Lista %s do webinar "%s". Criada automaticamente.', 'plugin-leads-saas' ), $label, $webinar_name ),
                'list_type'  => $type,
    ...
    public static function enter_inbox( int $webinar_id, int $contact_id ): void {
        ...
        PLS_Contact::subscribe_list( $contact_id, $inbox_id );

        do_action( 'pls_event', 'webinar_inbox', $contact_id, array( 'webinar_id' => $webinar_id ) );
    }
    ...
    public static function route_contact( int $webinar_id, int $contact_id, string $destination ): void {
        ...
        PLS_Contact::subscribe_list( $contact_id, (int) $dest_list->id );

        // Remove from INBOX (keep it empty).
        $inbox_id = self::get_inbox_list_id( $webinar_id );
        if ( $inbox_id ) {
            PLS_Contact::unsubscribe_list( $contact_id, $inbox_id );
        }

        // Fire routed event.
        do_action( 'pls_event', "webinar_{$destination}", $contact_id, array(
            'webinar_id'  => $webinar_id,
