...
class PLS_Automation_Engine {
    ...
    public static function trigger( string $event_type, int $contact_id, $data = array() ): void {
        global $wpdb;
        $table = PLS_Database::table( 'automations' );
    ...
    private static function start_automation( object $automation, int $contact_id, $data ): void {
        global $wpdb;
        $steps_table = PLS_Database::table( 'automation_steps' );
        ...
        $logs_table = PLS_Database::table( 'automation_logs' );
    ...
    public static function enqueue_step( int $automation_id, int $step_id, int $contact_id, ?string $execute_at = null ): void {
        ...
        $wpdb->insert(
            PLS_Database::table( 'automation_logs' ),
            array(
    ...
    private static function evaluate_condition( object $step, int $contact_id, array $config ): bool {
        ...
        switch ( $type ) {
            ...
            case 'contains_tag':
            case 'received_tag':
                $tag = PLS_Tag::get_by_name( $value );
                if ( ! $tag ) {
                    return false;
                }
                $tags = PLS_Contact::get_tags( $contact_id );
                foreach ( $tags as $t ) {
            ...
            case 'in_list':
                $lists = PLS_Contact::get_lists( $contact_id );
                foreach ( $lists as $l ) {
            ...
            case 'score_above':
                $contact = PLS_Contact::get( $contact_id );
            ...
            default:
                return (bool) apply_filters( 'pls_condition_evaluate', false, $type, $contact_id, $config );
        }
    }
    ...
    private static function execute_action( object $step, int $contact_id, array $config ): string {
        ...
        switch ( $action ) {
            case 'add_tag':
                ...
                PLS_Contact::add_tag( $contact_id, (int) $tag_id );
            ...
            case 'remove_tag':
                ...
                $tag      = PLS_Tag::get_by_name( $tag_name );
                if ( $tag ) {
                    PLS_Contact::remove_tag( $contact_id, (int) $tag->id );
                }
            ...
            case 'subscribe_list':
                ...
                if ( $list_id ) {
                    PLS_Contact::subscribe_list( $contact_id, $list_id );
                }
            ...
            case 'unsubscribe_list':
                $list_id = absint( $config['list_id'] ?? 0 );
                if ( $list_id ) {
                    PLS_Contact::unsubscribe_list( $contact_id, $list_id );
                }
            ...
            case 'send_email':
                ...
                if ( $template_id ) {
                    $tpl = PLS_Email_Template::get( $template_id );
                    if ( $tpl ) {
                        if ( empty( $subject ) ) {
                            $subject = $tpl->subject;
                        }
                ...
                if ( $contact ) {
                    PLS_Message_Queue::enqueue( $contact_id, 'email', $contact->email, $subject, $body, null, $template_id );
                }
            ...
            case 'send_whatsapp':
                ...
                if ( $contact && $contact->phone ) {
                    PLS_Message_Queue::enqueue( $contact_id, 'whatsapp', $contact->phone, '', $message );
                }
            ...
            case 'update_field':
                ...
                if ( $field ) {
                    PLS_Contact::update( $contact_id, array( $field => $val ) );
                }
            ...
            case 'update_score':
                ...
                if ( $contact ) {
                    ...
                    PLS_Contact::update( $contact_id, array( 'lead_score' => $new_score ) );
                }
            ...
            default:
                do_action( 'pls_action_execute', $action, $contact_id, $config );
    ...
    private static function get_next_step( object $current_step ): ?object {
        global $wpdb;
        $table = PLS_Database::table( 'automation_steps' );
    ...
    public static function get_automation( int $id ): ?object {
        global $wpdb;
        return $wpdb->get_row( $wpdb->prepare(
            'SELECT * FROM ' . PLS_Database::table( 'automations' ) . ' WHERE id = %d', $id
        ) ) ?: null;
    }
    ...
    public static function get_steps( int $automation_id ): array {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            'SELECT * FROM ' . PLS_Database::table( 'automation_steps' ) . ' WHERE automation_id = %d ORDER BY step_order ASC',
            $automation_id
        ) );
    }
    ...
    public static function delete_automation( int $id ): bool {
        global $wpdb;
        $wpdb->delete( PLS_Database::table( 'automation_steps' ), array( 'automation_id' => $id ), array( '%d' ) );
        $wpdb->delete( PLS_Database::table( 'automation_logs' ), array( 'automation_id' => $id ), array( '%d' ) );
        return (bool) $wpdb->delete( PLS_Database::table( 'automations' ), array( 'id' => $id ), array( '%d' ) );
    }

    public static function all_automations(): array {
        global $wpdb;
        return $wpdb->get_results( 'SELECT * FROM ' . PLS_Database::table( 'automations' ) . ' ORDER BY created_at DESC' );
    }

    public static function count_active(): int {
        global $wpdb;
        return (int) $wpdb->get_var( $wpdb->prepare(
            'SELECT COUNT(*) FROM ' . PLS_Database::table( 'automations' ) . ' WHERE status = %s', 'active'
        ) );
    }
}
