...
class PLS_Event_Manager {
    ...
    private function __construct() {
        add_action( 'pls_event', array( $this, 'handle' ), 10, 3 );
    }
    ...
    public function handle( string $event_type, int $contact_id, $data = array() ): void {
        ...
        // 2. Update lead score.
        PLS_Lead_Scoring::on_event( $event_type, $contact_id, $data );

        // 3. Trigger matching automations.
        PLS_Automation_Engine::trigger( $event_type, $contact_id, $data );
    }
    ...
    public static function get_recent( int $limit = 50 ): array {
        global $wpdb;
        $table = PLS_Database::table( 'events' );

        return $wpdb->get_results( $wpdb->prepare(
            "SELECT e.*, c.email, c.first_name, c.last_name
             FROM $table e
             LEFT JOIN " . PLS_Database::table( 'contacts' ) . " c ON c.id = e.contact_id
    ...
    public static function count_total(): int {
        global $wpdb;
        return (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . PLS_Database::table( 'events' ) );
    }
}
