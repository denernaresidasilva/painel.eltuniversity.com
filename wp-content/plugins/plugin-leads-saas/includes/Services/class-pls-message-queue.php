...
class PLS_Message_Queue {
    ...
    public static function enqueue( int $contact_id, string $channel, string $recipient, string $subject, string $body, ?string $scheduled_at = null, int $template_id = 0 ): int {
        ...
        $wpdb->insert(
            PLS_Database::table( 'message_queue' ),
            array(
    ...
    public static function process(): void {
        ...
        foreach ( $messages as $msg ) {
            ...
            if ( 'email' === $msg->channel ) {
                $success = PLS_Email::send( $msg );
            } elseif ( 'whatsapp' === $msg->channel ) {
                $success = PLS_WhatsApp::send( $msg );
            }
        ...
        if ( $channel ) {
            return $wpdb->get_results( $wpdb->prepare(
                "SELECT mq.*, c.email, c.first_name, c.last_name
                 FROM $table mq
                 LEFT JOIN " . PLS_Database::table( 'contacts' ) . " c ON c.id = mq.contact_id
                 WHERE mq.channel = %s
