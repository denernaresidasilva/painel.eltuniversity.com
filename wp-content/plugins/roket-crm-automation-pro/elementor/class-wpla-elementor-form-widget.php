...
class WPLA_Elementor_Form_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title(): string {
        return __( 'Roket CRM Form', 'roket-crm' );
    }
    ...
    public function get_keywords(): array {
        return array( 'form', 'crm', 'lead', 'subscribe', 'roket' );
    }

    protected function register_controls(): void {
        // Content section.
        $this->start_controls_section( 'content_section', array(
            'label' => __( 'Form Settings', 'roket-crm' ),
            'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
        ) );

        // List selector.
        $lists   = WPLA_List_Model::all();
        $options = array( 0 => __( '— Select List —', 'roket-crm' ) );
        foreach ( $lists as $list ) {
            $options[ $list->id ] = $list->name;
        }

        $this->add_control( 'list_id', array(
            'label'   => __( 'List', 'roket-crm' ),
        ...
        $this->add_control( 'form_title', array(
            'label'   => __( 'Title', 'roket-crm' ),
            'type'    => \Elementor\Controls_Manager::TEXT,
            'default' => __( 'Subscribe to our list', 'roket-crm' ),
        ) );

        $this->add_control( 'button_text', array(
            'label'   => __( 'Button Text', 'roket-crm' ),
            'type'    => \Elementor\Controls_Manager::TEXT,
            'default' => __( 'Subscribe', 'roket-crm' ),
        ) );
        ...
        $this->start_controls_section( 'style_section', array(
            'label' => __( 'Form Style', 'roket-crm' ),
            'tab'   => \Elementor\Controls_Manager::TAB_STYLE,
        ) );

        $this->add_control( 'button_color', array(
            'label'     => __( 'Button Color', 'roket-crm' ),
        ...
        $this->add_control( 'border_radius', array(
            'label'      => __( 'Border Radius', 'roket-crm' ),
    ...
    protected function content_template(): void {
        ...
        <div class="wpla-form-wrapper">
            ...
            <div style="padding:20px;background:#f9fafb;border-radius:8px;text-align:center;">
                <p style="color:#6b7280;">📋 Roket CRM Form — List #{{{ settings.list_id }}}</p>
