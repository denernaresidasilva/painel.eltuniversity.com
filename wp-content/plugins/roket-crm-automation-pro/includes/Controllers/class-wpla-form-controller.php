...
class WPLA_Form_Controller {
    ...
    public function render_shortcode( $atts ): string {
        $atts = shortcode_atts( array(
            'list'        => 0,
            'title'       => '',
            'button_text' => __( 'Subscribe', 'roket-crm' ),
            'class'       => '',
        ), $atts, 'wpla_form' );
        ...
        // Default form fields.
        $fields = array(
            array( 'name' => 'email', 'label' => __( 'Email', 'roket-crm' ), 'type' => 'email', 'required' => true ),
            array( 'name' => 'first_name', 'label' => __( 'First Name', 'roket-crm' ), 'type' => 'text', 'required' => false ),
            array( 'name' => 'last_name', 'label' => __( 'Last Name', 'roket-crm' ), 'type' => 'text', 'required' => false ),
            array( 'name' => 'phone', 'label' => __( 'Phone', 'roket-crm' ), 'type' => 'tel', 'required' => false ),
        );
    ...
    public function handle_submit(): void {
        ...
        if ( ! check_ajax_referer( 'wpla_form_nonce', 'nonce', false ) ) {
            wp_send_json_error( array( 'message' => __( 'Security check failed.', 'roket-crm' ) ), 403 );
        }
        ...
        if ( $count >= 10 ) {
            wp_send_json_error( array( 'message' => __( 'Too many submissions. Please try again later.', 'roket-crm' ) ), 429 );
        }
        set_transient( $trans, $count + 1, 60 );

        $email = isset( $_POST['email'] ) ? sanitize_email( wp_unslash( $_POST['email'] ) ) : '';
        if ( ! is_email( $email ) ) {
            wp_send_json_error( array( 'message' => __( 'Please enter a valid email.', 'roket-crm' ) ) );
        }
        ...
        wp_send_json_success( array( 'message' => __( 'Thank you for subscribing!', 'roket-crm' ) ) );
    }
}
