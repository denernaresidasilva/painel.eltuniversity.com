...
class WPLA_Webinar {
    ...
    public static function create( array $data ) {
        ...
        $automation_id = WPLA_Automation_Engine::save_automation( array(
            'name'         => sprintf( __( 'Automação — %s', 'roket-crm' ), $row['name'] ),
            'description'  => sprintf( __( 'Automação dedicada do webinar "%s". Gerada automaticamente.', 'roket-crm' ), $row['name'] ),
    ...
    private static function create_lists( int $webinar_id, string $webinar_name ): void {
        foreach ( self::LIST_TYPES as $type => $label ) {
            WPLA_List_Model::create( array(
                ...
                'description' => sprintf( __( 'Lista %s do webinar "%s". Criada automaticamente.', 'roket-crm' ), $label, $webinar_name ),
                'list_type'  => $type,
